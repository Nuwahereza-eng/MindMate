
export const lg = {
  appName: "MindMate", // Keep or translate if a common term exists
  tagline: "Omwoleso gwo obulamu bwo obw'omwoyo",
  // Nav
  navChat: "Kuba nange", // "Chat with me" or "Emboozi" (Conversation)
  navMood: "Enneewulira", // "Feelings"
  navJournal: "Ekiwandiiko kyange", // "My writings"
  navWellness: "Obulamu Obulungi", // "Good Health/Well-being"
  navTherapists: "Abasawo b'emitwe", // "Head doctors" (colloquial for therapists)
  navPremium: "Eby'omuwendo", // "Things of value"
  navSettings: "Entegeka", // "Arrangements/Settings"
  navEmergencySupport: "Obuyambi obw'Emergency",
  // Auth
  signIn: "Yingira", // "Enter"
  signUp: "Weewandiise", // "Register yourself"
  signInWithPhone: "Yingira n'Essimu",
  signUpWithPhone: "Weewandiise n'Essimu",
  firstNameRequired: "Erinnya ery'oluberyeberye lyetaagisa okuwandiika n'essimu.",
  welcomeBack: "Tukwanirizza nate", // "We welcome you again"
  createAccount: "Gulawo akawunti", // "Open an account"
  firstName: "Erinnya Ery'oluberyeberye",
  lastName: "Erinnya Ery'enkomerero",
  email: "Esimu/Bbaruwa",
  emailOrPhone: "Esimu/Bbaruwa oba Namba y'Essimu",
  phoneNumberLabel: "Namba y'Essimu",
  password: "Ekigambo Eky'ekyama",
  needAccount: "Wetaaga akawunti? Weewandiise",
  haveAccount: "Olina akawunti? Yingira",
  continueGoogle: "Yingirira ne Google",
  signInSuccessGoogle: "Oyingidde ne Google bulungi!",
  signInErrorGoogle: "Okuyingira ne Google Kulemeredde",
  signInSuccessPhone: "Oyingidde n'Essimu bulungi!",
  logout: "Fuluma",
  loggedOutSuccessfully: "Ofulumye bulungi",
  logoutFailed: "Okuggyamu Kulemeredde",
  myAccount: "Akawunti Yange",
  sendVerificationCode: "Weereza Koodi y'Okuwuliziganya",
  verificationCode: "Koodi y'Okuwuliziganya",
  verificationCodeSent: "Koodi y'okuwuliziganya esindikiddwa ku ssimu yo!",
  errorSendingCode: "Ensobi mu kusindika koodi. Kebera namba ogezeeko nate.",
  errorVerifyingCode: "Ensobi mu kukakasa koodi. Gezaako nate.",
  enterVerificationCodeTitle: "Yingiza Koodi y'Okuwuliziganya",
  enterVerificationCodeMessage: "Tusindise koodi ya namba 6 ku {{phoneNumber}}. Giyingize wansi.",
  verifyAndCreateAccount: "Kakasa & Gulawo Akawunti",
  verifyAndSignIn: "Kakasa & Yingira",
  recaptchaError: "Ensobi ya reCAPTCHA. Gezaako nate.",
  recaptchaExpired: "reCAPTCHA eggwaako. Gezaako nate.",
  recaptchaNotReady: "reCAPTCHA tetennategeka. Linda akaseera.",
  recaptchaNotReadyOrFailed: "reCAPTCHA tetennategeka oba yalemwa. Gezaako nate oba kakasa nti etegekeddwa bulungi.",
  recaptchaNotReadyOrFailedShort: "reCAPTCHA si ntegefu. Gezaako nate.",
  enterPhoneNumberToVerify: "Kakasa {{phoneNumber}}",
  enterPhoneNumberToVerifyE164: "Tujja kutuma koodi y'okukakasa ku {{phoneNumber}}. Ebisale ebya bulijjo biyinza okukozesebwa.",
  ensureCountryCode: "Kakasa nti namba yo ey'essimu erimu koodi y'eggwanga (okugeza, +12223334444).",
  ensureE164Format: "Kozesa enkola ya E.164 (okugeza +12223334444).",
  back: "Ddayo",
  user: "Omukozesa",
  tryAgainLater: "Gezaako oluvannyuma.",
  invalidPhoneNumberFormat: "Enkola ya namba y'essimu si ntuufu. Teekamu koodi y'eggwanga lyo (okugeza, +1234567890).",
  invalidPhoneNumberFormatE164: "Namba y'essimu si ntuufu. Kozesa enkola ya E.164 (okugeza +12223334444).",
  invalidPhoneNumberFirebase: "Namba y'essimu tewandiikiddwa bulungi.",
  tooManyRequestsFirebase: "Tuziyizza okusaba kwonna okuva ku kyuma kino olw'ebikolwa ebitategeerekeka. Gezaako oluvannyuma.",
  invalidVerificationCodeFirebase: "Koodi y'okukakasa si ntuufu. Gezaako nate.",
  codeExpiredFirebase: "Koodi y'okukakasa eggwaako. Saba endala.",
  networkError: "Ensobi y'Omutimbagano",
  checkYourConnection: "Kebera omutimbagano gwo ogezeeko nate.",
  // Chat
  chatPlaceholder: "Wandika obubaka bwo...",
  signInToChat: "Yingira okusobola okunyumya",
  botGreeting: "Mbalamusizza! Nze MindMate, mukwano gwo akufaako mu by'obulamu bw'omwoyo. Otya leero?",
  crisisWarning: "Ndi mweraliikirivu nnyo olw'ebyo by'ogambye. Enneewulira yo ntegeerekeka, naye njagala okakasa nti ofuna obuyambi bw'owetaagawo. Funa abakugu mu kuyamba abali mu bizibu.",
  feelingAnxiousResponse: "Ntegedde owulira nga oli mweraliikirivu. Kino kya bulijjo. Oyagala nkuyise mu kazannyirizi k'okussa obulungi, oba oyagala twogere ku ekyo ekikweraliikiriza?",
  feelingSadResponse: "Mpulira nti oli mu kiseera kizibu. Kyetaagisa obuvumu okwogera ku nneewulira eno. Kijjukire nti kino kya kiseera buseera. Kiki ekiyinza okukuyamba leero?",
  feelingStressedResponse: "Okuwulira ng'olemereddwa kitegeerekeka nnyo. Ka tukitwale mpola. Oyagala tukole akazannyirizi k'okwekkakkanya oba twogere ku ekyo ekikulemesa?",
  offerBreathingExerciseResponse: "Kyetusaliddewo kirungi! Obuzannyirizi bw'okussa buyamba nnyo. Nkusaba ogezeeko enkola ya 4-7-8: ssa omukka munda okutuuka ku 4, guwummuze okutuuka ku 7, oluvannyuma gufulumye okutuuka ku 8. Dding'ana emirundi 3-4. Oyagala nkuyiseemu?",
  defaultResponse1: "Weebale kunyumya nange. Okugezaako bino kukuwulizisa otya?",
  defaultResponse2: "Nsiimye okwogera kw'oliko. Kiki ekiyinza okukuyamba kati?",
  defaultResponse3: "Kivuga ng'olina ekintu ekikulu. Osobola okunnyonnyola engeri kino gyekikukwatamu?",
  defaultResponse4: "Enneewulira zo za mazima. Buyambi ki obwandibadde obw'omugaso gy'oli leero?",
  defaultResponse5: "Ndi wano okuwuliriza. Kiki ekikubaluseeko mu biro bino?",

  moodTrackerTitle: "Okulondoola Enneewulira",
  howAreYouFeelingToday: "Owulira otya leero?",
  saveMood: "Tereka Enneewulira",
  moodHistory: "Ebyafaayo by'Enneewulira",
  noMoodEntriesYet: "Tewali nneewulira ziterekeddwa. Londoola enneewulira yo okulaba enkyukakyuka.",

  journalTitle: "Ekiwandiiko Kyange",
  writeTodaysEntry: "Wandiika Ebya Leero",
  journalPlaceholder: "Owulira otya? Kiki ekiri ku mutima gwo leero?",
  saveEntry: "Tereka Ekiwandiiko",
  previousEntries: "Ebiwandiiko Ebyayita",
  noJournalEntries: "Tewali biwandiiko. Tandika okuwandiika okulondoola endowooza n'enneewulira zo!",

  wellnessExercisesTitle: "Obuzannyo bw'Obulamu Obulungi",
  breathingExercises: "Obuzannyo bw'Okussa",
  dailyAffirmations: "Ebigambo Eby'okwezzamu Amaanyi",
  personalizedAffirmations: "Ebigambo Eby'okwezzamu Amaanyi Eby'omuntu",
  generateMyAffirmations: "✨ Funkuna Ebigambo Byange Eby'okwezzamu Amaanyi",
  generatingAffirmations: "Nkwata Ebigambo...",
  affirmationsGeneratedSuccess: "Ebigambo by'okwezzamu amaanyi bitegekeddwa!",
  noAffirmationsGenerated: "Situsobodde kufuna bigambo bya kwezzamu maanyi. Gezaako edda.",
  errorFetchingAffirmations: "Wabaddewo nsobi mu kufuna ebigambo by'okwezzamu amaanyi. Gezaako nate.",
  affirmationsGenerationError: "Okufuna Ebigambo Kugaanye",
  clickToGenerateAffirmations: "Nyiga waggulu okufuna ebigambo by'okwezzamu amaanyi okusinziira ku bikolwa byo ebisembyeyo.",
  startExercise: "Tandika Oluzannyo",
  exerciseFeatureComingSoon: "Enkola y'okuzannya eno ejja mangu!",
  breathingExercise478Name: "Okussa kwa 4-7-8",
  breathingExercise478Duration: "Ddakiika 2",
  breathingExercise478Desc: "Ssa omukka munda okubala 4, kuwummuze 7, fulumya 8",
  breathingExerciseBoxName: "Okussa kw'Akasanduuko",
  breathingExerciseBoxDuration: "Ddakiika 3",
  breathingExerciseBoxDesc: "Okussa, okuwummuza, okufulumya, okuwummuza byenkana",
  breathingExerciseBellyName: "Okussa Okw'omubiri",
  breathingExerciseBellyDuration: "Ddakiika 5",
  breathingExerciseBellyDesc: "Essa ku kusimba olubuto lwo",
  affirmation1: "Oli wa maanyi okusinga bw'olowooza",
  affirmation2: "Enneewulira eno yaakuggwaawo",
  affirmation3: "Osanidde okwagala n'essanyu",
  affirmation4: "Buli mukka gw'ossa buwanguzi",
  affirmation5: "Wawangudde ebizibu emabega",

  therapistsTitle: "Yogerako n'Abasawo",
  bookSession: "Weereza Olukiiko",
  therapistRating: "Okusiimwa:",
  therapistSessionPrice: "Olukiiko:",
  howItWorks: "Enkola yaakyo",
  howItWorksPoint1: "Noonya abasawo abakakasiddwa mu kitundu kyo",
  howItWorksPoint2: "Buuka enkiiko ezikugyamu",
  howItWorksPoint3: "Emikutu gya vidiyo emikakafu oba enkiiko mu bantu",
  howItWorksPoint4: "Abasawo bonna bakakasiddwa era balina layisinsi",

  premiumMembershipTitle: "Obunywani obw'Omuwendo",
  upgradeNow: "Longoosa Kati",
  freePlan: "Pulani ya Bwereere",
  perMonth: "/omwezi",
  premiumPlan: "Pulani y'Omuwendo",
  freePlanFeatures: [
    "Obuyambi bwa chatbot obusookerwako",
    "Okulondoola enneewulira",
    "Ekiwandiiko ekyangu",
    "Obuzannyo bw'okussa obusookerwako",
  ],
  freePlanMissingFeatures: [
    "Okwekenneenya okw'omulembe",
    "Okukwatagana n'abasawo",
    "Ebintu eby'omuwendo",
    "Ebigambo by'okwezzamu amaanyi eby'omuntu",
  ],
  premiumPlanFeatures: [
    "Byonna ebiri mu Bwereere",
    "Okwekenneenya enneewulira okw'omulembe",
    "Ebiwandiiko ebitaliiko kikomo",
    "Okwefumiitiriza okw'omuwendo okulagirirwa",
    "Ebigambo by'AI Eby'omuntu Eby'okwezzamu Amaanyi",
    "Okutumira abasawo obubaka obutereevu",
    "Obuyambi obusookeddwa",
  ],
  currentPlan: "Pulani Yange",
  moneyBackGuarantee: "Garantiya y'okuzzaayo ssente mu nnaku 30",
  moneyBackGuaranteeDesc: "Gezaako Premium awatali kutya. Bw'oba tommatidde, tujja kukuzzaayo ssente zo zonna mu nnaku 30.",
  premiumUpgradeSuccess: "Weekoledde! Obunywani bwo obwa MindMate Premium butandise!",

  settingsTitle: "Entegeka",
  appearance: "Endabika",
  darkMode: "Enzikiza Mode",
  language: "Olulimi",
  privacySecurity: "Eby'obukuumi n'Obwama",
  exportMyData: "Fuluumya data yange",
  deleteAllData: "Sazaamu data yonna",
  notifications: "Obubaka",
  dailyCheckIns: "Okwekebejja Okwa Buli Lunaku",
  moodReminders: "Okujjukiza ku Nneewulira",

  crisisSupportTitle: "Obuyambi mu Bizibu Eby'amaanyi",
  crisisSupportMessage: "Bw'oba n'ebirowoozo eby'okwetta oba okwetulugunya, funa obuyambi mangu:",
  ugandaCrisisLine: "Layinoni y'Obuyambi mu Uganda",
  internationalCrisisLine: "Layinoni y'Obuyambi mu Nsi Yonna",
  continueChat: "Yongera Okunyumya",
  callNow: "Kuba Essimu Kati",

  joined: "Yeegasse ku",
  loading: "Etikka...",
  error: "Wabaddewo ensobi. Gezaako nate.",
  submit: "Weereza",
  close: "Galawo",
  freeMember: "Mmemba wa Bwereere",
  premiumMember: "Mmemba ow'Omuwendo",
  theme: "Endabika",
  accessPremiumFeature: "Funa Eby'omuwendo",
  upgradeToAccess: "Olina okulongoosa okufuna {{feature}}.",
};
